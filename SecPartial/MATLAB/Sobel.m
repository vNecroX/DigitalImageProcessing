...
A = imread("yulin.jpg");
imgr = escalagris(A);
figure('Name','Filtro Sobel', 'NumberTitle', 'off')
subplot(2, 2, 1);
...
function filtrar_sobel(imgr)
    ...
    for r=2:m-1
        for c=2:n-1
            Gx(r, c) = -1*suave(r-1, c-1) - 2*suave(r, c-1) - 1*suave(r+1, c-1) + ...
            suave(r-1, c+1) + 2*suave(r, c+1) + suave(r+1, c+1);
            Gy(r, c) = -1*suave(r-1, c-1) - 2*suave(r-1, c) - 1*suave(r-1, c+1) + ...
            suave(r+1, c-1) + 2*suave(r+1, c) + suave(r+1, c+1);
        end
    end
